...
def update_database():
    try:
        ...
        # Iterate over the fetched IP addresses
        for ip_address in ip_addresses:
            # Fetch Username, Password, and port for the current IP address
...
def update_dvr(ip_address, username, password, port):
    try:
        ...
        if ping_status == 'Working':
            ...
            # Update DVR time in IST in database
            if dvr_time_data:
                update_dvr_time_status(dvr_time_data, cursor, conn, ip_address)
                login_status = 'Working'

        else:
            # If ping is timeout, update Camera, HDD, and Login status to NULL or Not Working
